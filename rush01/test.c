...
int *convert_row_tab(char *str) {
    ...
    // Parcourir la chaîne pour obtenir les valeurs des lignes (après les colonnes)
    while (str[i] != '\0' && j < 8) {
        // On commence à regarder à partir du 16e caractère, en ignorant les espaces
        if (str[i + 16] >= '1' && str[i + 16] <= '4') {  // Décalage de 16 pour ignorer les colonnes
